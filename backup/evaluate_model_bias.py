...
class PrimeKGDrugRepurposingGNN(nn.Module):
    ...
    def score(self, z, pairs, degrees):
        src_idx = pairs[0]
        dst_idx = pairs[1]
...
def load_assets():
    ...
    deg_path = Path("models/degrees.pt")
    csv_path = Path("data/primekg.csv")

    for p in (meta_path, model_path, adj_path, deg_path, csv_path):
        if not p.exists():
    ...
    degrees = torch.load(deg_path, map_location=device)
    ...
    return metadata, model, adj, z, df, device, degrees
...
def test_global_popularity(metadata, model, z, device, degrees):
...
def test_jaccard_diversity(metadata, model, z, device, degrees, n_pairs=50):
    """
    ...
    def get_topk(dis_idx):
        ...
        with torch.no_grad():
            sc = torch.sigmoid(model.score(z, pairs, degrees)).cpu().numpy()
...
def test_hub_filtered_reranking(metadata, model, z, drug_degree, device, degrees):
    """
...
def main():
    metadata, model, adj, z, df, device, degrees = load_assets()

    # Test 1
    drug_degree = test_degree_census(metadata, df)

    # Test 2
    drug_global = test_global_popularity(metadata, model, z, device, degrees)

    # Test 3
    rho, pval, spearman_verdict = test_spearman(drug_degree, drug_global)

    # Test 4
    avg_jaccard, jaccard_verdict = test_jaccard_diversity(metadata, model, z, device, degrees)

    # Test 5
    emb_gap = test_embedding_similarity(metadata, z, drug_degree, device)

    # Test 6
    test_hub_filtered_reranking(metadata, model, z, drug_degree, device, degrees)
